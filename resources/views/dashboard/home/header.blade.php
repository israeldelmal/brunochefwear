		...
		<div>
			...
			<form action="{{ url('/escritorio/cabecera/actualizar') }}" method="POST" enctype="multipart/form-data">
				<div>
					...
					<input type="text" name="h1" id="h1" autocomplete="off" placeholder="Título de la cabecera" value="{{ $header->h1 }}">
					@if ($errors->has('h1'))
				...
				<div>
					<label for="p">Párrafo</label>
					<textarea name="p" id="p" rows="5" placeholder="Párrafo de la cabecera">{{ $header->p }}</textarea>
					@if ($errors->has('p'))
						<div>{{ $errors->first('p')}}</div>
					@endif
				</div>
				<div>
